...
function uvPyramid(baseWidth, height, slices, noBottom) {
   ...
   height = height || 1;
   slices = slices || 17;  // Default number of sides now set to 17

   var vertexCount = (slices + 1);  // Vertex count for the apex and the base perimeter vertices
   if (!noBottom) vertexCount += slices + 1;  // Additional vertices for the bottom if it is not open
   var triangleCount = slices;  // Number of triangles for the sides
   if (!noBottom) triangleCount += slices;  // Additional triangles for the bottom
   ...
   // Apex of the pyramid at the top
   vertices[kv] = 0; normals[kv++] = 0;  // x
   vertices[kv] = 0; normals[kv++] = 0;  // y
   vertices[kv] = height / 2; normals[kv++] = 1;  // z
   texCoords[kt++] = 0.5;  // s
   texCoords[kt++] = 0.5;  // t
   ...
   for (i = 0; i < slices; i++) {
      ...
      var c = Math.cos(u);
      var s = Math.sin(u);
      vertices[kv] = halfWidth * c; normals[kv++] = 0;
      vertices[kv] = halfWidth * s; normals[kv++] = 0;
      vertices[kv] = -height / 2; normals[kv++] = -1;
      texCoords[kt++] = 0.5 + 0.5 * c;
      texCoords[kt++] = 0.5 + 0.5 * s;
   }
   ...
   for (i = 0; i < slices; i++) {
      indices[k++] = 0;  // Apex
      indices[k++] = i + 1;
      indices[k++] = i + 2 > slices ? 1 : i + 2;
   }

   // Bottom cap
   if (!noBottom) {
      var startIndex = kv / 3;  // Start index for bottom vertices
      vertices[kv] = 0; normals[kv++] = 0;
      vertices[kv] = 0; normals[kv++] = 0;
      vertices[kv] = -height / 2; normals[kv++] = -1;
      texCoords[kt++] = 0.5;
      texCoords[kt++] = 0.5;
      ...
      for (i = 0; i < slices; i++) {
         indices[k++] = startIndex;
         indices[k++] = startIndex + i + 1;
         indices[k++] = startIndex + i + 2 > startIndex + slices ? startIndex + 1 : startIndex + i + 2;
      }
   }
